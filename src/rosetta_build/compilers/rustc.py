"""rustc compiler adapter (compile and link are typically one invocation)."""
...
__all__ = ["RustcCompiler", "RustcLinker"]


class RustcCompiler(ArgvCompiler):
    @property
...
class RustcLinker(ArgvLinker):
    """Placeholder linker; rustc usually emits the final artifact itself."""
    ...
    def argv_for_link(self, request: LinkRequest) -> list[str]:
        raise NotImplementedError(
            "rustc uses a single compile invocation; separate link is unsupported"
        )
